      ...
      <div className="mx-auto max-w-6xl px-4 pt-8">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px]">
          <header>
            ...
            <div className="mt-6 flex items-center gap-4">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <h1 className="text-lg font-semibold text-ink">{guide.full_name}</h1>
                  <TierBadge tier={guide.tier} />
                </div>
                <p className="text-sm text-ink-soft">
                  {guide.home_district}, Nepal
                  {guide.years_experience ? (
                    <>
                      {" "}· <span className="font-mono">{guide.years_experience}</span> years guiding
                    </>
                  ) : null}
                </p>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-5 lg:hidden">
              {messageForm(
                `Message ${first} — free`,
            ...
            <section className="mt-12 space-y-4">
              {routeChips.length > 1 && (
                <div id="routes" className="scroll-mt-6">
                  <p className="label text-muted">Routes {first} runs</p>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {routeChips.map((r: any) => (
                      <Link
                        ...
                        to={`/routes/${r.slug}`}
                        prefetch="intent"
                        className="rounded-pill border border-line bg-card px-3 py-1 text-sm text-ink hover:border-sage"
                      >
                        {r.name}
                        {r.count > 0 && (
